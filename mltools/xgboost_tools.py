...
class XGBoostParamsOptimizer:
    ...
    def _optimize_trees(self, trials, max_evals, max_depth_min, max_depth_max, min_eta, max_eta, eta_step,
        ...
        lamb = 1.0 if default_lambda else hp.qloguniform('lambda', np.log(min_lambda), np.log(max_lambda), 1)
        alpha = 0.0 if zero_alpha else hp.qloguniform('alpha', np.log(min_alpha), np.log(max_alpha), 1)
        space = {
    ...
    def get_optimal_exact_xgb_params(self, features, target, folds, objectives, score_metric, extra_metrics=[],
